...
class HomeDashboardActivity : AppCompatActivity() {
    ...
    private fun render() {
        ...
        val shieldButton = when {
            mode == GuardianVpnService.Mode.TRACKER_SHIELD -> "STOP SHIELD"
            !shieldReady -> "CHOOSE APPS"
            else -> "START SHIELD"
        }
        root.addView(controlCard(
            "Tracker Shield",
            if (shieldReady) "$shieldCount app(s) selected · filters visible tracker DNS" else "Choose which apps Guardian should shield.",
            mode == GuardianVpnService.Mode.TRACKER_SHIELD,
            shieldButton,
            green
        ) {
            when {
                mode == GuardianVpnService.Mode.TRACKER_SHIELD -> stopVpn()
                !shieldReady -> openProtectionApps(ProtectionAppsActivity.MODE_SHIELD)
                else -> requestVpnAndStart(GuardianVpnService.ACTION_TRACKER_SHIELD)
            }
        }.top(dp(9)))

        val firewallButton = when {
            mode == GuardianVpnService.Mode.FIREWALL -> "STOP FIREWALL"
            firewallCount == 0 -> "CHOOSE APPS"
            else -> "START FIREWALL"
        }
        root.addView(controlCard(
            "Smart Firewall",
            if (firewallCount > 0) "$firewallCount app(s) selected · blocks their network traffic" else "Choose which apps Guardian should block.",
            mode == GuardianVpnService.Mode.FIREWALL,
            firewallButton,
            violet
        ) {
            when {
                mode == GuardianVpnService.Mode.FIREWALL -> stopVpn()
                firewallCount == 0 -> openProtectionApps(ProtectionAppsActivity.MODE_FIREWALL)
                else -> requestVpnAndStart(GuardianVpnService.ACTION_FIREWALL)
            }
        }.top(dp(10)))

        root.addView(controlCard(
            "Emergency Lock Down",
            "Stops network traffic through Guardian until you restore it.",
            mode == GuardianVpnService.Mode.LOCKDOWN,
            if (mode == GuardianVpnService.Mode.LOCKDOWN) "RESTORE NETWORK" else "LOCK DOWN NOW",
            red
        ) {
        ...
        root.addView(MaterialButton(this).apply {
            text = "MANAGE PROTECTION APPS"
            textSize = 12f
            minHeight = dp(44)
            setOnClickListener { openProtectionApps(ProtectionAppsActivity.MODE_FIREWALL) }
        }.top(dp(11)))
        ...
        root.addView(actionCard(
            ...
            if (profiles.isEmpty()) "Shield one app at a time to build an evidence-based Privacy Profile." else "${profiles.size} app profile(s) · ${NumberFormat.getIntegerInstance().format(totalBlocks)} classified tracker request(s) blocked.",
            "VIEW PRIVACY PROFILES"
        ) {
        ...
        root.addView(MaterialButton(this).apply {
            text = "ADVANCED NETWORK CONTROLS"
            setOnClickListener { openAdvancedControls() }
        }.top(dp(12)))
        root.addView(text(
            "Guardian reports only what local evidence supports. Tracker Shield does not decrypt HTTPS or claim complete visibility into encrypted DNS, cached addresses, or direct-IP traffic.",
            11f,
            secondary,
            Typeface.NORMAL
        ).top(dp(18)))
    ...
    private fun requestVpnAndStart(action: String) {
        ...
        if (prepare != null) {
            ...
            startActivityForResult(prepare, 901)
        } else {
            startVpn(action)
        }
    }
    ...
    private fun openProtectionApps(mode: String) {
        startActivity(Intent(this, ProtectionAppsActivity::class.java).putExtra(ProtectionAppsActivity.EXTRA_MODE, mode))
    }
    ...
    private fun hero(title: String, body: String, accent: Int) = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
    ...
    private fun controlCard(title: String, subtitle: String, active: Boolean, buttonText: String, accent: Int, onClick: () -> Unit) = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
        setPadding(dp(16), dp(15), dp(16), dp(14))
        background = rounded(if (active) raised else surface, 18f, if (active) accent else null)
        addView(text(if (active) "$title · ACTIVE" else title, 18f, if (active) accent else primary, Typeface.BOLD))
        addView(text(subtitle, 12f, secondary, Typeface.NORMAL).top(dp(5)))
    ...
    private fun actionCard(title: String, subtitle: String, buttonText: String, onClick: () -> Unit) = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
    ...
    private fun card(value: String) = text(value, 14f, primary, Typeface.NORMAL).apply {
    ...
    private fun compact(value: Long) = when {
